...
void MyGLCanvas::draw() {
    ...
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    switch (gm->GetInstance()->GetGameState()) {
        case 0:
...
void MyGLCanvas::drawScene() {
    ...
    glUniform1i(
            glGetUniformLocation(obstacleShaderID, "wallTexture"), 1
    );
    glUniform1i(
            glGetUniformLocation(obstacleShaderID, "wallNormalTexture"), 2
    );
    glUniformMatrix4fv(
    ...
    for (auto &o: gm->GetInstance()->obs) {
        ...
        glUniform3fv(
                glGetUniformLocation(obstacleShaderID, "obstacle_color"), 1, glm::value_ptr(gm->GetInstance()->enumToColor(o.color))
        );
        glUniform3fv(
                glGetUniformLocation(obstacleShaderID, "obstacle_delta"), 1, glm::value_ptr(o.pos)
        );
        o.poly->renderVBO(myShaderManager->getShaderProgram("obstacleShader")->programID);
    }
...
void MyGLCanvas::initShaders() {
    ...
    myShaderManager->addShaderProgram("playerShaders", "shaders/330/player-vert.shader",
                                      "shaders/330/player-frag.shader");
    playerPLY->buildArrays();
    playerPLY->bindVBO(myShaderManager->getShaderProgram("playerShaders")->programID);

    myShaderManager->addShaderProgram("obstacleShader", "shaders/330/obstacle-vert.shader",
                                      "shaders/330/obstacle-frag.shader");
    for (auto &o: gm->GetInstance()->obs) {
        o.poly->buildArrays();
        o.poly->bindVBO(myShaderManager->getShaderProgram("obstacleShader")->programID);
    }
    glEnable(GL_BLEND);
